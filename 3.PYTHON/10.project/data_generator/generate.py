...
def generate():
    try:
        ...
        if data_type.lower() == 'order' and (os.path.isfile("csv/user.csv") == False or os.path.isfile("csv/store.csv") == False):
            raise InputError("user.csv 혹은 store.csv 파일이 없습니다.")
        elif data_type.lower() == 'orderitem' and (os.path.isfile("csv/item.csv") == False or os.path.isfile("csv/order.csv") == False):
    ...
    if prnt_type.lower() == 'screen':
        ...
        my_printer.print_to_file(g_datas.data, f"csv/{data_type.lower()}.csv")
